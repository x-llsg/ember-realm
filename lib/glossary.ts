...
export const HELP = {
  ...
  mastery: {
    title: '专精培养',
    body: '专精最高 5 级，倍率为 1 + 专精等级 × 4%。它同时提升原生生命、攻击和防御，满级为 1.20 倍，个人六槽装备随后另加。需要 2 级酒馆；出征角色归来后可培养，留守角色照常培养。升至 M 级基础花费 80M² 金、40M 粮、12M 铁；隐修者各项乘0.85后向上取整。',
  },
  xp: {
    title: '经验',
  ...
  levels: {
    title: '等级与训练',
    body: '六个城镇阶段的等级上限为 6 / 10 / 16 / 24 / 32 / 40，城镇进阶会提高上限。训练支付金币与口粮后直接升一级，已有经验保持原数值。L 级训练基础花费为向上取整的 30 + 20L^2.1 金，以及 20 + 10L 粮；勤勉先把金币费用乘 0.85 再取整。上述训练费用再乘1–5星系数0.4 / 0.55 / 0.75 / 1 / 1.2；临时便餐另减25%口粮，公会训练抵扣自动使用。退役返还真实支付训练费用的80%为专用抵扣，免费经验不返还。新招旅人的初始等级为 max(1, 已击败首领数 × 3)，出身不直接增加等级，但可改变训练费用和经验。',
  },
  origins: {
    title: '出身',
  ...
  tier: {
    title: '装备阶级',
    body: 'T 阶装备的基础攻、血、防倍率为 1.75^(T−1)，再乘品质、强化和工匠传承倍率。各城镇阶段最高可制作 T1 / T1 / T2 / T3 / T4 / T6，已经开放的低阶仍可选择。阶级不放大抗性、穿甲或远程比例；高阶费用还需要工艺和地区材料。',
  },
  enhancement: {
    title: '强化',
  ...
  affix: {
    title: '装备词条',
    body: `每件新装备从 ${G.AFFIXES.length} 种词条中等机会抽取一种。锋锐提供额外攻击，坚韧提供额外生命，二者乘装备完整数值倍率；其余词条按标明的固定值加算。支付 20 × 装备阶级的锻造尘可直接选择另一词条，不需要再次随机。词条已经计入装备详情与角色属性。`,
  },
  dust: {
    title: '锻造尘',
    body: '拆解闲置装备获得 装备阶级 × 品质序号 × 4 点锻造尘，品质序号从朴素 1 到神话 6。正在穿戴的装备必须先卸下，尘库存上限为 9999。装备库满 120 件时，新获得的远征装备会自动转为锻造尘；超出尘上限的部分不会储存。锻造尘用于定向重铸，不返还原制作材料。',
  },
  kit: {
    title: '全队行装',
  ...
  resistance: {
    title: '元素抗性',
    body: '火、暗、神抗分别加算，个人每种上限 75%，队伍抗性取出战成员的算术平均。首领战只使用敌人的对应元素抗性，抵抗掉该比例伤害；物理敌人不读取这三种抗性。同元素战前附魔再给决战抗性 +20 个百分点，总上限仍为 75%。据点有效战力使用队伍对应抗性，不计这项战前附魔。',
  },
  intelligence: {
    title: '地区敌情',
    body: '各地区敌情独立保存，上限 100。每点给据点推进成功率增加 0.15 个百分点，并向首领伤害基础倍率增加 0.001，100 点对应 +10%。调查每趟获得 4 + 野外学术等级 × 2 点，出战成员中有博闻再加 3 点。敌情只是优势，0敌情也可挑战已通路首领；工程线索单独计算。',
  },
  frontier: {
    title: '据点推进',
  ...
  supply: {
    title: '补给运输',
    body: '补给用于反复运回基础物资和地区材料，不推进据点或增加调查线索。出发门槛满足后按 100% 归来处理，普通、丰收、伏击、宝匣仍会改变基础物资收益。地区材料每趟为 floor((4 + 2×当地深度) × 车队运量)，运量为 1 + floor((集市等级 + 仓库等级)÷4)，驿站补给网再乘 1.5。地区材料不乘丰收或伏击倍率，超过材料仓容量的部分无法带回。',
  },
...
const AFFIX_DETAIL: Record<(typeof G.AFFIXES)[number]['stat'], string> = {
  crit: '向个人暴击率增加8个百分点；个人暴击率上限60%。暴击默认造成150%伤害。',
  dodge:
    '向个人闪避率增加6个百分点；个人闪避上限40%。只能闪避敌方单体攻击，群体重击不能闪避。',
  critDamage:
    '个人暴击倍率增加0.20，例如150%变为170%。只放大暴击命中的直接伤害。',
  attack:
  ...
  pierce:
    '向个人穿甲增加 12 个百分点，不随装备成长放大；超过个人 75% 上限的部分不生效。',
  fire: '向个人火抗增加 15 个百分点，不随装备成长放大；个人上限 75%，全队取出战成员平均。',
  shadow:
    '向个人暗抗增加 15 个百分点，不随装备成长放大；个人上限 75%，全队取出战成员平均。',
  radiant:
    '向个人神抗增加 15 个百分点，不随装备成长放大；个人上限 75%，全队取出战成员平均。',
};
...
export function affixHelp(index: number, s?: G.State, item?: G.Gear): HelpText {
  ...
  if (s && item) {
    ...
    const value = ['pierce', 'fire', 'shadow', 'radiant'].includes(affix.stat)
...
const TALENT_DETAIL: Record<(typeof G.TALENTS)[number]['id'], string> = {
  ...
  warden: '分别增加火、暗、神抗各 20 个百分点，与装备相加后个人各项最高 75%。',
  healer:
    '补给治疗增加的是治疗目标最大生命的 5 个百分点，需本人出战，同天赋多人不重复叠加。',
