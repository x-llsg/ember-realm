...
export const HELP = {
  ...
  rarity: {
    title: '装备品质',
    body: '装备白/绿/蓝/紫/金/红六档，数值倍率1 / 1.15 / 1.30 / 1.45 / 1.65 / 1.85。锻造概率50% / 30% / 15% / 4% / 1%，不会出红；每4件至少1件蓝装。守敌节点越深，掉装率和高品质占比越高；第三守敌首次胜利必掉至少蓝装，再战不重复保底。首领必掉当地套装，红装仅首领掉落。选择敌人后，悬停掉落说明可查看该节点的实际概率。套装件数与品质独立，可混搭。',
  },
  craftPity: {
    title: '每四次锻造保底',
  ...
  tier: {
    title: '装备阶级',
    body: `装备是独立于潜力的成长来源。T1–T6 同配方的攻、血、防相对 T1 倍率为 ${Array.from({length:6},(_,i)=>(G.gearTierScale(i+1)/G.gearTierScale(1)).toFixed(2)).join(' / ')}，再乘品质、强化和工匠传承倍率；预览直接显示实际数值。各城镇阶段最高可制作 T1 / T1 / T2 / T3 / T4 / T6，已经开放的低阶仍可选择。掉落由首领战绩和本区阶级共同决定：未击败首领时，各地只掉T1；击败任意分支首领可提高全局掉落上限，并提升当地新掉落阶级，最高T6，首胜战利品即生效。例如森林通关后，森林与已发现的中期地区可掉T2；旧图仍受本区上限限制，不会一路升到T6。已有装备阶级保持原样。阶级不放大抗性、穿甲或远程比例；高阶强化与制作仍需要工艺和地区材料。`,
  },
  enhancement: {
    title: '强化',
