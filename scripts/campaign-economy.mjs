...
const outputDir = new URL(`../.test-results/v14-${routeOrder}-balanced/`, import.meta.url);
...
const results = {
  version: 10,
  rulesRevision: 'v14-balanced-parallel-economy-with-goal-monitor',
  visitorDeferred: [],
  visitorChoices: [],
  researchInvestments: [],
  gearOrders: [],
...
function allocate() {
  const n = s.population-G.transportWorkers(s);
  const desired =
    n < 6
...
const previousGoal=activeResourceGoal; activeResourceGoal={...cost};
try {
...
let activeResourceGoal={},activeMaterialGoal={};
const demandPacing={sampling:'Actual waiting-state samples, each interval <=5 game seconds; stock fullness is estimated, not exact overflow.',materialVisibleSeconds:{},materialFullSeconds:{},usefulAffordableSeconds:0,openMapUsefulSeconds:0,openMapSeconds:0,samples:0,usefulOptions:{},longestNoUsefulSeconds:0,currentNoUsefulSeconds:0,allocationPolicy:'Original balanced allocation and stage investment, monitored against actual pending bills without changing decisions.'};
function demandBill(){
...
function improveAffordable(id,purpose){
  ...
  const cost=G.developmentCost(s,id),beforeRate=G.production(s),beforeWork=d.work?G.workDuration(s,d.work):null;
...
function economyOpportunity(cost,purpose){
  const deficits=Object.entries(cost).filter(([k,n])=>s.resources[k]+1e-8<n).sort((a,b)=>(b[1]-s.resources[b[0]])/(G.production(s)[b[0]]||.01)-(a[1]-s.resources[a[0]])/(G.production(s)[a[0]]||.01));
  if(deficits.length){improveAffordable(deficits[0][0],purpose);return;}
  if(purpose==='stage development'){
    for(const id of ['forestry','agriculture','masonry','commerce','smelting','attunement','education','storage'])improveAffordable(id,purpose);
  }
}
